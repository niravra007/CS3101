...
typedef struct book{//bookdata struct
    char title[255];
    char id[50];
    char author[50];
    int no;
...
typedef struct bookuser{//userdata struct
    char title[255];
    char id[50];
    char author[50];
    char date[50];
...
typedef struct userpass{//userpass data struct
    char username[20];
    char password[20];
} USER;


void fileRead(char *filename)//function to read bookdata file
{
    FILE *fptr;
    BOOK book;
...
void fileReadUser(char *filename)//function to read userfiles
{
    FILE *fptr;
    BOOKUSER bookuser;
...
void fileSearch(char *filename, char *usertitle, char *userauthor, char *userid)
{
    ...
    while (fread(&book, sizeof(BOOK), 1, fptr)) {
        // Lowercase conversions for fields to ensure case-insensitive matching
...
void fileWrite(char *filename, BOOKUSER* books, int no_of_records)//function to write into userfiles
{
    FILE *fptr;
    int i;
...
void fileWriteBook(char *filename, BOOK* books, int no_of_records)//function to write into bookdata file
{
    FILE *fptr;
    int i;
...
void deleteBookEntry( char *deleteID)
{
    ...
    books = (BOOK*)malloc((no_of_rec-1)*sizeof(BOOK));//dynamic allocation of space for the required number of records 
    
    rewind(fptr);//go to the start of the bookdata file
    ...
    if(countdel==0){
        ...
        FILE *adminl = fopen("adminlog.txt","a");
                time_t now = time(NULL);//using the time module to log the deletion of the book
                struct tm *local = localtime(&now);
...
void fileFacultyIssue(const char *username, char *bookID) {
    // Open the book database file to find book details
    FILE *bookDB = fopen("bookdata.txt", "r+b");
    if (bookDB == NULL) {
        printf("Error opening book database!\n");
        exit(1);
    ...
    now += days_to_issue * 24 * 60 * 60;  // Add the days to local time
